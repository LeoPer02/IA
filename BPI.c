...
struct node{                              // Nós de uma linked list para criar uma queue
    short int *grid;
    struct node *next;
};
...
typedef struct queue{                             // Implementação de uma queue usando Linked List
    int size;
    node *front;
    node *back;
}queue;

void inicializar(queue *q){                       // Inicializar a queue
    q->size = 0;
    q->front = NULL;
    q->back = NULL;
}

bool isEmpty(queue *q){                           // Verificar se a queue está vazia ou não
    return (q->size == 0);
}
...
int main(){
    short int grid1[16], grid2[16];                                   // short int para minimizar espaço em memória
    for(int i = 0; i < 16; i++){scanf("%hd", &grid1[i]);}             // Ler inteiros para dentro do array grid
    for(int i = 0; i < 16; i++){scanf("%hd", &grid2[i]);} 
    ...
    if(possivel(grid1, grid2) == 0){                                  // Mensagem de erro caso não seja possivel ir de Si -> Sf
        printf("Não é possivel chegar de um estado ao outro\n");    
        return 0;
    }else{                                                            // Iniciar processo de busca
...
int inversions(short int* grid){
    ...
    for(int i = 0; i < 16; i++){
        for(int j = i+1; j < 16; j++){
            if(*(grid + i) > *(grid + j) && *(grid+j) != 0 && *(grid + i) != 0){ count++; }
        }
    }
    return count;
}
...
bool possivel(short int* grid1, short int* grid2){
    ...
    int zero1, zero2;
    for(int i = 0; i < 16; i++){
        if(*(grid1+i) == 0){zero1 = i;}
        if(*(grid2+i) == 0){zero2 = i;}
    }
